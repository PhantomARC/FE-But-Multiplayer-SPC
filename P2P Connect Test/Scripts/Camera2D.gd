...
func _process(delta):
	...
	if Input.is_action_just_released("left_click"):
		...
		var index_map = get_tree().get_root().get_node("MainScene/"+ \
				get_tree().get_root().get_node("MainScene").mapnode) \
				.world_to_map(mouse_pos)
		set_position(get_tree().get_root().get_node("MainScene/"+ \
				get_tree().get_root().get_node("MainScene").mapnode) \
				.map_to_world(index_map) + Vector2(32,32))
...
func refer_tile(returnfunc):
	...
	if not tilecalc.size() == 4:
		tilecalc.insert(2,"0")
	var reference = Vector2(alpha_numdic[tilecalc[0]] * 26 \
			+ alpha_numdic[tilecalc[1]],
			(int(tilecalc[2]) * 10 + int(tilecalc[3])) * -1)
	set_position(get_tree().get_root().get_node("MainScene/"+ \
			get_tree().get_root().get_node("MainScene").mapnode) \
			.map_to_world(reference) + Vector2(32,32))
